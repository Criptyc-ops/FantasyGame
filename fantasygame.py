...
class Game:
    ...
    def fast_travel(self):
        """Fast travel between discovered locations with a mount"""
        if not self.player.has_mount:
            print(Colors.YELLOW + "\nYou don't have a mount yet. Rent one at the Royal Stables." + Colors.RESET)
            return

        destinations = self.player.discovered_locations
        if len(destinations) <= 1:
            print(Colors.YELLOW + "\nNo other discovered locations are available for fast travel." + Colors.RESET)
            return

        print(Colors.GREEN + Colors.BOLD + "\n=== FAST TRAVEL ===" + Colors.RESET)
        print("Choose a destination:")
        for i, loc_id in enumerate(destinations, 1):
            loc = locations[loc_id]
        ...
        try:
            ...
            if 0 <= idx < len(destinations):
                ...
                if destination == self.player.current_location:
                    print(Colors.YELLOW + "\nYou are already here." + Colors.RESET)
                    return

                self.player.current_location = destination
                if destination not in self.player.visited_locations:
                    self.player.visited_locations.append(destination)

                print(Colors.CYAN + f"\nYou ride your mount to {locations[destination]['name']}." + Colors.RESET)
                self.display_location()
        except ValueError:
            print(Colors.YELLOW + "\nInvalid travel choice." + Colors.RESET)

    def show_crafting(self):
        """Show crafting recipes"""
        print(Colors.ORANGE + Colors.BOLD + "\n=== CRAFTING ===" + Colors.RESET)
        print("Recipes:")
        print("  healing potion - 2 Forest Herbs + 1 Glowing Mushroom")
        print("  leather armor - 1 Wolf Pelt + 1 Troll Hide")
        print("  knight's sword - 1 Iron Ore + 1 Silver Ore")
        print("\nUse 'craft [recipe]' to craft an item.")
        print()
    ...
    def craft_item(self, recipe_name):
        ...
        recipes = {
            'healing potion': {
                'ingredients': {'forest_herbs': 2, 'glowing_mushroom': 1},
                'result': 'healing_potion'
            },
            'leather armor': {
                'ingredients': {'wolf_pelt': 1, 'troll_hide': 1},
                'result': 'leather_armor'
            },
            "knight's sword": {
                'ingredients': {'iron_ore': 1, 'silver_ore': 1},
                'result': 'sword'
            },
            'knights sword': {
                'ingredients': {'iron_ore': 1, 'silver_ore': 1},
        ...
        if recipe_key not in recipes:
            print(Colors.YELLOW + f"\nUnknown recipe: '{recipe_name}'." + Colors.RESET)
            print("Type 'craft' to see available recipes.")
            return
        ...
        for ingredient, required_count in ingredients.items():
            owned_count = self.player.inventory.count(ingredient)
            if owned_count < required_count:
                missing.append((ingredient, required_count - owned_count))

        if missing:
            print(Colors.YELLOW + "\nYou don't have the required ingredients:" + Colors.RESET)
            for ingredient, shortage in missing:
                print(f"  - {items[ingredient]['name']} x{shortage}")
            return

        for ingredient, required_count in ingredients.items():
            for _ in range(required_count):
                self.player.remove_item(ingredient)

        result_key = recipe['result']
        self.player.add_item(result_key)
        print(Colors.GREEN + f"\nYou crafted {items[result_key]['name']}!" + Colors.RESET)
        if items[result_key]['type'] == 'consumable':
            print(Colors.GREEN + f"{items[result_key]['name']} has been added to your inventory." + Colors.RESET)
    ...
    def show_help(self):
        ...
        print("  travel/mount - Fast travel between discovered locations")
